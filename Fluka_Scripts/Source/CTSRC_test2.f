...
*     
      INTEGER LMAX, L, POSTN, CNT,PHIST 
      PARAMETER (LMAX=120)
      REAL*8 LOC_X, LOC_Y, LX, LY, PERL, PITCH, PI2
      DIMENSION LOC_X(LMAX), LOC_Y(LMAX)
      SAVE L, LOC_X, LOC_Y, POSTN, PHIST, CNT, PERL
*	  
      INTEGER B, H
      REAL*8 B_HEIGHT,CUM_B,MAGVEC,HCA,TUB2ISO,TABFEED,TOTROT,TABMOV
      REAL*8 Z_DIST, CUM_H, H_BIN, B_BIN, ZDIST
...
*
      ...
      SAVE N, ERG, E_MAX, CE, SLT, T, ZDIST
*
...
**** Read BOWTIE Profile File ****
 ...
 260   CONTINUE 
         ...
         CLOSE (LUNRD)
       WRITE(*,*) 'BOWTIE File Read: B_BIN =',B_BIN
**** Read spectrum ***
       LUNRD = NINT(WHASOU(7))
       WRITE(*,*) 'LUNRD is ', LUNRD
       N = 0
...
*       WRITE(*,*) 'E2 = ', E, 'CE = ', CE(N,1)
 ...
 460   CONTINUE
       ...
       END IF
**** Check Location File ***
       IF((WHASOU(10) .GT. ZERZER) .AND. (WHASOU(1) .EQ. ZERZER)) THEN
*  |*** Location file called ***|
	   LUNRD = NINT(WHASOU(10))
       WRITE(*,*) 'LUNRD is ', LUNRD
       L = 0
       ROT = 0
 170   CONTINUE
       READ(LUNRD, *, ERR=9999, END=180) LY, LX
*	   WRITE(*,*) 'Reading Location File ',L
        L = L+1
        LOC_X (L) = LX
        LOC_Y (L) = LY
       GOTO 170
 180   CONTINUE 
       WRITE(*,*) 'LOcation File Read'
       CLOSE (LUNRD)
	   END IF
**** Check TCM File ***
...
* Initialize Variables for uniform rotation	   
	   PHIST = NINT(WHASOU(11))
	   PERL = PHIST/L
	   CNT = 0
	   POSTN = 1
	   END IF
